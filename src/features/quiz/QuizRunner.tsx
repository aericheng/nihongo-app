...
function SpellAnswer({
  ...
  return (
    <div className="space-y-3">
      {/* 作答顯示區：內容全部來自下方內嵌鍵盤，不使用 input，系統鍵盤不會出現 */}
      <div className="flex min-h-[3.25rem] items-center rounded-lg border border-slate-300 bg-slate-50 px-3 py-2">
        {value === '' ? (
          <span className="text-sm text-slate-400">用下方鍵盤拼出答案</span>
        ) : (
          <span className="font-jp text-xl font-semibold text-slate-800">{value}</span>
        )}
      </div>

      <KanaKeyboard value={value} disabled={!answering} onChange={onChange} />
